...
const normalizeClassBreaks = require('./normalize-class-breaks')
...
function calculateClassBreaks (features, classification) {
  ...
  const values = classification.normType ? normalizeClassBreaks(features, classification) : getFieldValues(features, classification.field)
  ...
  return classifyClassBreaks(values, classification)
...
function classifyClassBreaks (values, classification) {
  const {
    breakCount,
    method
  } = classification
