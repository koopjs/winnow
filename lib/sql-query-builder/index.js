...
function params (features, { sourceDataCoordinateSystem, outputCrs: { wkid, wkt }, aggregates, geometry, geometryPrecision }) {
  const params = []
  // NOTE: order matters here
  // select fragment: transform function parameters here
  if (!aggregates && (wkid || wkt)) {
    params.push(sourceDataCoordinateSystem, wkt || `EPSG:${wkid}`)
  }
